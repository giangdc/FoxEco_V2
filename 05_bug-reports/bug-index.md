...
> 🔄 **Sync gần nhất: 2026-09-24 — `/sync-jira-bugs` (PULL, không cờ lọc).**
> JQL: `project = FE AND issuetype = Bug ORDER BY key ASC` ⇒ **phạm vi = TOÀN BỘ project FE, 124 bug** (`FE-93` … `FE-350`).
...
| Cần gì | Đọc ở đâu |
|---|---|
| Bug đã push Jira (mirror, trạng thái mới nhất) | `05_bug-reports/jira/<KEY>-*.md` (glob đệ quy) — **124 file**, ghi đè toàn bộ mỗi lượt sync, ⛔ KHÔNG sửa tay |
