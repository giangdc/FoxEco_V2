...
| SC-ORD-008 | Giá trị hàng bắt buộc *(UI: "GIÁ TRỊ HÀNG (ƯỚC TÍNH)")* | REQ-ORD-005 | DOC-v1.0-01 §D8.1 L359 · DOC-v1.0-06 KP-01 §10.4 · DOC-v1.1-03 ảnh `gia tri hang uoc tinh.png` | Đang ở Bước 1/3, chưa chọn Giá trị hàng (**3 chip, không chip nào được chọn** — ⛔ không phải dropdown như PRD v1.1 `§8.1.4` ghi: `C-ORD-18`) | Bấm "Tiếp theo" | Nút "Tiếp theo" ở trạng thái disabled/mờ, KHÔNG chuyển bước. ⚠️ Ảnh demo 2026-09-17 cho thấy nút **vẫn cam/active** khi chưa chọn gì — nghi là giới hạn ảnh tĩnh, **vibe-check bấm thật** trước khi kết luận, ⛔ không log bug theo mỗi ảnh | P2 | Business Rule | NEW |
| SC-ORD-009 | Cảnh báo Giá trị "Cao" | REQ-ORD-005 | DOC-v1.0-01 §D8.1 L359 · DOC-v1.0-06 KP-01 §3 KB-ORD-05 · DOC-v1.1-03 ảnh `gia tri hang uoc tinh.png` | Đang ở Bước 1/3 | Chọn chip Giá trị hàng = **"Cao"** *(nhãn phụ trên UI: "Trên 5 triệu đ")* | Hiện banner nguyên văn: "Hàng giá trị cao: hai bên tự thoả thuận và chịu trách nhiệm với nhau. FoxEco không bảo hiểm, không đứng ra vận chuyển hay bồi thường." | P2 | Business Rule | NEW |
| SC-ORD-010 | Không cảnh báo với Thấp/Vừa | REQ-ORD-005 | DOC-v1.0-06 KP-01 §3 KB-ORD-05 · DOC-v1.1-03 ảnh `gia tri hang uoc tinh.png` | Đang ở Bước 1/3 | Chọn chip Giá trị hàng = **"Thấp"** *(Dưới 1 triệu đ)*, sau đó = **"Vừa"** *(1 – 5 triệu đ)* | Cả 2 trường hợp KHÔNG hiện banner cảnh báo. ⛔ **GHI NHẬN** mốc tiền hiển thị, **không assert cứng** con số (PRD chưa có — `C-ORD-18`) | P3 | Business Rule | NEW |
| SC-ORD-011 | Ghi chú — biên 300 ký tự | REQ-ORD-004 | DOC-v1.0-01 §D8.1 L358 · DOC-v1.1-03 ảnh `gia tri hang uoc tinh.png` | Đang ở Bước 1/3 — ô **GHI CHÚ nằm ở block ĐẦU TIÊN**, trước "Thông tin hàng" (`C-ORD-18`); placeholder *"Lưu ý khi giao nhận (VD: gọi trước khi tới, giao tại sảnh, hàng dễ vỡ nhẹ tay...)"* | Nhập Ghi chú 300 ký tự, rồi thử 301 ký tự | 300 ký tự: hợp lệ, qua bước được. 301: bị chặn/không nhập thêm được | P3 | Business Rule | NEW |
| SC-ORD-012 | Ảnh sản phẩm — ràng buộc *(UI: "ẢNH HÀNG \*")* | REQ-ORD-006 | DOC-v1.0-01 §D8.1 L360 · DOC-v1.1-03 ảnh `gia tri hang uoc tinh.png` | Đang ở Bước 1/3 — khối ảnh có dấu `*` bắt buộc, vùng thả + icon camera, **bộ đếm "0/5"**, helper *"Bắt buộc ít nhất 1 ảnh · tối đa 5 ảnh · giúp người vận chuyển nhận diện hàng"* | Thử tải: 1 ảnh JPG 4MB · 1 ảnh PNG 5MB · 1 ảnh >5MB · 1 file không phải JPG/PNG · thử tải ảnh thứ 2 | 3 case đầu (≤5MB, JPG/PNG) được nhận; >5MB và sai định dạng bị chặn; chỉ giữ được 1 ảnh duy nhất | P3 | Business Rule | NEW |
...
| SC-ORD-019 | Email không tra thấy | REQ-ORD-008 | DOC-v1.0-01 §D1b US-D18 L168 · §D8.1 L366 | Có email đúng định dạng nội bộ nhưng KHÔNG tồn tại trong danh bạ | Nhập email đó, rời ô | Hiện thông báo "Không tìm thấy · nhập thủ công"; 3 trường người nhận để trống cho user tự nhập. ⛔ **HẾT HIỆU LỰC TỪ v1.1 (2026-09-17)** — BA `C-ORD-14` chốt **chặn tạo đơn** (nút "Tiếp theo" disable), ⛔ KHÔNG mở ô nhập tay. Bản hiện hành: **`SC-ORD-059`** (`v1.1/ORD-dang-tin/`). ⛔ Đừng chạy SC này cho v1.1 | P2 | Functional | NEW |
...
> 🖼️ **Bổ sung 2026-09-17 — bằng chứng UI demo** (`00_input/v1.1/design/gia tri hang uoc tinh.png`, QC cung cấp): nhãn field đầy đủ là **"GIÁ TRỊ HÀNG (ƯỚC TÍNH)"** và 3 mức có **mốc tiền** — `Thấp` *(Dưới 1 triệu đ)* · `Vừa` *(1 – 5 triệu đ)* · `Cao` *(Trên 5 triệu đ)*. **Con số này KHÔNG có ở nguồn nào trước đó** (cả `§D8.1` v1.0 lẫn `§8.1.4` v1.1) ⇒ **✅ BA chốt 2026-09-17 là CON SỐ CHÍNH THỨC ⇒ ASSERT CỨNG** (`C-ORD-18` Resolved; rào *"ghi nhận, chưa assert cứng, chờ BA chốt"* của bản trưa cùng ngày **HẾT HIỆU LỰC**). Đề nghị BA bổ sung vào `§8.1.4`; tới khi PRD cập nhật, nguồn chốt là câu trả lời BA + ảnh `DOC-v1.1-03` (`v1.1/ORD-dang-tin/risk_assessment.md C-ORD-18`). ⚠️ **Không lẫn với `C-ORD-02`** — đây là **nhãn phụ mô tả 3 mức định tính**, KHÔNG phải ô nhập số tiền hay ngưỡng cấu hình của `BR-ORD-03` (vẫn out of scope).
