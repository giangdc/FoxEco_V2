...
| RISK-CNL-07 | CNL / Trạng thái treo | **(risk mới)** `AC-25.2.01` nói đơn `INCIDENT` *"không tự về COMPLETED, phải qua admin hỗ trợ"*, nhưng **không mục nào của PRD mô tả bề mặt Admin** ⇒ nếu app build đúng spec thì đơn vào `INCIDENT` sẽ **treo vĩnh viễn** từ phía người dùng cuối | **High** | `DOC-v1.1-01` §6.2 AC-25.2.01 · §8.11 dòng Actor — đối chiếu toàn văn §8: không có §nào đặc tả Admin | `SC-CNL-015` (đơn không tự đóng) + `SC-CNL-017` (không có bề mặt Admin) | ⚠️ **Nêu với PM như gap nghiệp vụ trước khi execute** — đây là câu hỏi thiết kế, không phải bug code | Open | REQ-CNL-008, REQ-CNL-009, SC-CNL-015, SC-CNL-017 |
...
| C-CNL-03 | Ai thực hiện được "huỷ không thường" / đóng đơn `INCIDENT`, và qua bề mặt nào? | 🔴 **Open (non-blocking)** | 2026-09-15 | REQ-CNL-008, REQ-CNL-009, SC-CNL-015, SC-CNL-017 |
...
### C-CNL-03 · "Huỷ không thường" và đóng đơn INCIDENT — ai làm, qua đâu? *(OPEN)*
...
3. **Nêu `RISK-CNL-07` với PM trước khi execute** — nếu app build đúng `AC-25.2.01` mà không có đường tới admin thì đơn `INCIDENT` treo vĩnh viễn. Đây là câu hỏi thiết kế, phát hiện được ở khâu phân tích thì rẻ hơn nhiều so với phát hiện ở UAT.
