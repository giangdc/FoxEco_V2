...
- **Cập nhật 2026-09-17 (BA trả lời `C-HOME-04` vòng 3 — *"giữ nguyên nhé"*):** `SC-HOME-025` bỏ nốt rào chữ — **assert cứng** chuỗi `EMP-01` *"Chưa có tin nào trong khu vực của bạn"* dù danh sách load toàn quốc (BA chốt giữ nguyên chữ PRD). `HOME` **hết điểm treo BA**
...
|-------------|---------|--------|-----------|-------|------|------|----------|-----------|-----------|
| SC-HOME-019 | "Tin mới" — đúng 5 tin, loại trừ MATCHED/EXPIRED/của chính mình | REQ-HOME-011 | DOC-v1.1-01 §6.2 AC-11.1.01 | Toàn hệ thống có 12 tin NEED ở POSTED *(2026-09-17: BA chốt tin load **toàn quốc**, không lọc khu vực — `C-HOME-04` Resolved)*, trong đó có ít nhất 1 tin đã MATCHED, 1 tin đã EXPIRED và 1 tin do chính người dùng đăng | Mở Trang chủ | Section "Tin mới" hiển thị **đúng 5 tin** mới nhất; tin MATCHED, EXPIRED và tin của chính người dùng **KHÔNG** nằm trong danh sách; có nút "Xem thêm trên Bảng tin" | P3 | Business Rule | MODIFIED |
