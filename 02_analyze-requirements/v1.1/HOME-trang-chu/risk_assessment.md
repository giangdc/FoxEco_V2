...
| C-HOME-01 | Icon vai trò ở header (mapping) + lệch tagline banner giữa 2 doc | 🟡 **Partially Resolved 2026-09-15** — (b) tagline chốt; (a) icon mapping còn cần xác nhận STG | kế thừa từ v1.0 (mở 2026-09-07) | REQ-HOME-002, REQ-HOME-003 |
...
### C-HOME-01 · Icon vai trò header + tagline banner *(PARTIALLY RESOLVED 2026-09-15)*
...
**(a) Icon mapping theo vai trò — CHƯA RESOLVED, chỉ có thêm dữ liệu đầu vào (đúng như khuyến nghị `vibe-test 3 vai để có ảnh icon làm đầu vào`):**
> Cùng 3 ảnh trên — icon góc trái header (hình xe + hộp) **giống hệt nhau ở cả 3 vai trò** (Sender/Carrier/Receiver), không có mapping icon↔vai như `DOC-v1.0-06 KP-05 §3 dòng 5` mô tả.

**Analyst Note:** (a) là bằng chứng **phủ định** giả thuyết "icon khác nhau theo vai" của `KP-05`, nhưng KHÔNG đủ để khẳng định dứt khoát đây là **thiết kế đúng** (có thể demo đơn giản hoá, thiếu asset icon riêng cho vai) hay **thiết kế sai/thiếu** (build chưa cập nhật). Giữ Status "Partially Resolved": `SC-HOME-004` vẫn viết dạng ghi nhận (không assert icon cụ thể theo vai) cho tới khi verify trên STG thật; nhưng KHÔNG còn cần hỏi BA cho phần (b) tagline — dùng thẳng text PRD.
...
> ⚠️ `C-HOME-01` (icon vai trò header + tagline banner) và `C-HOME-02` (điều kiện ẩn "Đơn của tôi") **vẫn Open** — PRD v1.1 không đề cập tới 2 điểm này. Xem `v1.0/HOME-trang-chu/risk_assessment.md`.
...
5. **`C-HOME-01(b)` (tagline) không còn là blocker** — dùng text PRD "Tiện đường — Giúp đồng nghiệp". **`C-HOME-01(a)` (icon mapping) vẫn Partially Resolved** — `SC-HOME-004` giữ dạng ghi nhận, verify lại trên STG thật trước khi automation.
