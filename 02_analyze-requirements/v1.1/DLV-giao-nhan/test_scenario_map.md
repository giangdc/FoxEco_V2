...
| SC-DLV-039 | Xác nhận giao hàng — gửi quầy (lễ tân/bảo vệ) | REQ-DLV-017 | DOC-v1.1-01 §8.7.1 BR07-03 · BA `C-DLV-08` 2026-09-17 | Đơn ở IN_TRANSIT | Chọn "Quầy lễ tân" (hoặc "Quầy bảo vệ"), nhập **bắt buộc** "Họ tên người trực quầy" + "Số điện thoại" (khối `NGƯỜI NHẬN TẠI QUẦY *`, không chấp nhận ghi chung chung kiểu "quầy lễ tân"), đính ≥1 ảnh, xác nhận | Đơn chuyển DELIVERED; nhật ký ghi "Đã gửi tại quầy lễ tân — {tên}" (hoặc quầy bảo vệ tương ứng). ⛔ **Không assert** cờ "không liên lạc được" / `NTF-05` ⟷ `NTF-11` — **ghi nhận** câu log ở block LỊCH SỬ (`C-DLV-08` (a)(b) chờ vibe-check). ⛔ **Không assert** hạn giữ hàng 4h/24h — **out of scope v1.1**, role admin chưa build (`C-DLV-08` (d), BA 2026-09-17) | P2 | Business Rule | NEW |
...
| SC-DLV-043 | Không liên lạc được — thứ tự ưu tiên 4 nhánh | REQ-DLV-018 | DOC-v1.1-01 §8.8.1 BR08-02 | Đơn ở IN_TRANSIT, Carrier bấm "Không liên lạc được người nhận?" | Quan sát thứ tự các lựa chọn hiển thị trên màn | Đúng thứ tự: (1) giao cho người uỷ quyền đã khai sẵn (nếu có) → (2) liên hệ người gửi xin uỷ quyền → (3) gửi quầy lễ tân/bảo vệ → (4) cầm hàng về. ✅ **Chốt 2026-09-17 (`C-DLV-06` Resolved — vibe-check demo + BA vòng 3):** cấu trúc thật là **4 TẦNG MÀN** (⛔ không phải modal 2 hướng của `AC-16.1.01`): "Xác nhận đã giao" → nút *"Không thể liên lạc cho người nhận?"* → **MÀN** "Liên hệ người gửi" → "Xử lý đơn hàng". `AC-16.1.01`/`AC-17.1.01` là **PRD chưa cập nhật** ⇒ **assert theo cấu trúc 4 tầng**, ⛔ không còn dừng ở GHI NHẬN | P2 | UI | NEW |
...
| SC-DLV-045 | Gửi quầy từ luồng không liên lạc được | REQ-DLV-018 | DOC-v1.1-01 §8.8.1 BR08-04 · BA `C-DLV-08` 2026-09-17 | Từ luồng không liên lạc được, chọn "Gửi tại quầy" | Nhập tên người trực quầy + SĐT + ≥1 ảnh, xác nhận popup | Đơn chuyển DELIVERED qua nhánh này; bắt buộc đủ ảnh + tên + SĐT, có popup xác nhận trước khi ghi. ⛔ **Không assert** cờ/`NTF` phân biệt với `SC-DLV-039` — **ghi nhận** câu log block LỊCH SỬ (`C-DLV-08` (a)(b)). ⛔ **Không assert** hạn giữ hàng 4h/24h — out of scope v1.1 (role admin chưa build) | P2 | Business Rule | NEW |
...
| SC-DLV-048 | Thời hạn giữ hàng tại quầy — 4h nhắc, cuối ngày nhắc lần 2 | REQ-DLV-018 | DOC-v1.1-01 §8.8.1 BR08-06 · BA `C-DLV-07`/`C-DLV-08` 2026-09-17 | Đơn đã gửi tại quầy, chưa được Receiver xác nhận | Chờ 4 giờ, sau đó tới cuối ngày | Sau 4h: **có thông báo nhắc** gửi Receiver. Cuối ngày: nhắc lần 2 — **ghi nhận** kênh (push/in-app) + nguyên văn câu nhắc + giờ thực tế (BA không đặc tả "cuối ngày" là mấy giờ ⇒ **không assert giờ cụ thể**). ⛔ Vế **"sau 24h chuyển admin hỗ trợ" OUT OF SCOPE v1.1** — role admin chưa build phase này (BA 2026-09-17), **không assert, không log bug** khi không thấy. GHI DẠNG DEFERRED, cần tiền đề thời gian thực (cùng nhóm khó `RISK-DLV-04` cũ); chưa chạy đủ mốc thì GHI RÕ, không khai coverage | P3 | Business Rule | NEW |
...
| SC-DLV-050 | Chế độ retry — quay lại IN_TRANSIT theo lịch hẹn | REQ-DLV-019 | DOC-v1.1-01 §8.9.1 BR09-05 | Đơn ở RESCHEDULED, đã tới lịch hẹn giao lại | Carrier mở lại màn giao hàng theo lịch | Đơn quay lại IN_TRANSIT; giao lại theo đúng luồng `REQ-DLV-017` (FR07). ✅ **Chốt 2026-09-17 (`C-DLV-05` Resolved):** **CHÍNH Carrier** tự nhập khối "Hẹn trả hàng" (Ngày hẹn + Từ/Đến giờ + Nơi nhận lại) rồi giao lại — ⛔ hệ thống KHÔNG tự chuyển đúng giờ. Timeline giữ **5 mốc cố định**, `RESCHEDULED` **không có nhãn phụ** trên timeline | P2 | Functional | NEW |
...
| SC-DLV-054 | Quá lịch hẹn 24h chưa đóng — đơn KHÔNG tự đóng *(vế chuyển admin out of scope)* | REQ-DLV-019 | DOC-v1.1-01 §8.9.1 BR09-04 · AC-23.2.01 · BA `C-DLV-07` 2026-09-17 | Đơn ở RESCHEDULED hoặc RETURNING, đã quá lịch hẹn | Chờ thêm 24 giờ mà đơn chưa đóng | Đơn **KHÔNG tự chuyển "Hoàn thành"** và **KHÔNG đổi trạng thái/nhãn** nào khác; nếu có thì chỉ có **thông báo nhắc** (ghi nhận kênh + nguyên văn). ⛔ Vế **"chuyển admin hỗ trợ" OUT OF SCOPE v1.1** — role admin chưa build phase này (BA 2026-09-17), **không assert, không log bug**. GHI DẠNG DEFERRED, cần tiền đề thời gian thực; chưa chạy đủ mốc thì GHI RÕ | P3 | Business Rule | NEW |
...
| SC-DLV-060 | Admin can thiệp cũng được ghi log | REQ-DLV-021 | DOC-v1.1-01 §8.12.1 BR12-07 | Đơn đang ở trạng thái cần admin hỗ trợ (vd INCIDENT quá hạn) | Admin thực hiện can thiệp (ngoài phạm vi test — Admin Portal out of scope `C-TS-01`) | GHI NHẬN hệ quả quan sát được phía end-user: có dòng log mới xuất hiện tương ứng với can thiệp — KHÔNG verify được thao tác Admin thật | — | Business Rule | **DEPRECATED** *(descope 2026-09-17: BA chốt mọi bề mặt **role admin** OUT OF SCOPE v1.1 — `AC-23.2.01`/`BR08-06`/`BR09-04`. SC không còn bề mặt nào end-user quan sát được ⇒ 0 TC là ĐÚNG. ⛔ KHÔNG viết assert, KHÔNG log bug khi không thấy. Mở lại khi Admin Portal vào scope)* |
...
**Analyst Note:** `SC-DLV-048` cùng nhóm tiền đề khó với `RISK-DLV-04` cũ (`SC-DLV-024`, cần chờ giờ thật hoặc seed timestamp) — gộp kế hoạch chạy nếu có thể. **Cập nhật 2026-09-17 (BA `C-DLV-07`/`C-DLV-08`):** phạm vi assert thu hẹp còn *"có thông báo nhắc"* + *"đơn không tự đóng"*; **mọi vế "chuyển admin hỗ trợ" / hạn giữ hàng do admin xử lý là OUT OF SCOPE v1.1** (role admin chưa build). Áp chung cho `SC-DLV-024` (v1.0) · `SC-DLV-048` · `SC-DLV-054`.
