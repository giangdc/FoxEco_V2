...
|---|---|---|---|---|
| 2026-09-17 | UPDATE | **BA đính kèm ảnh UI mới** `00_input/v1.1/design/gift_tang_qua_man_doncuatoi.png` — màn "Đơn của tôi" tab "Đã hoàn thành": card đã tặng quà có badge "🎖 Đã tặng quà", card chưa tặng có dòng "Chạm để tặng quà". Bổ sung test data cho card list, KHÔNG phải ảnh nút "Bạn đã đánh giá" trong màn Tặng quà (nợ #4 vẫn còn treo, khác vị trí UI) | Rà lại `CL-hoi-BA-v1.1.xlsx` theo yêu cầu QC 2026-09-17 (phát hiện ảnh mới BA đính kèm chưa được xử lý) | Không đổi SC/TC counts; thêm ghi chú test data ở `risk_assessment.md` |
| 2026-09-17 | UPDATE | **Áp câu trả lời BA vòng 2 `C-GIFT-04`** → **Resolved**: (1) vào lại tặng sau vẫn được, KHÔNG thời hạn ⇒ giải toả mâu thuẫn `C-ACT-01` (câu (b) lượt trước hiểu đúng là "không thời hạn", không phải "không cho tặng sau"); (2) nút đổi "Bạn đã đánh giá" + disable; (3) tap card đã tặng → "Theo dõi đơn"; (4) `NTF-06` chưa tặng → "Tặng quà", đã tặng → "Theo dõi đơn". Vibe-check demo 2026-09-17 không tới được trạng thái Hoàn thành để đối chứng ảnh UI (ô tải ảnh bằng chứng không mở được file picker trong demo) — không ảnh hưởng việc đóng CL | BA trả lời vòng 2 `CL-hoi-BA-v1.1.xlsx` sheet `GIFT` 2026-09-17 · vibe-check demo QA GiangDC2 (Playwright MCP) | `counts` cl_resolved 3→4, cl_open giữ 0 (không còn CL Partially); `SC-GIFT-005`, `SC-ACT-009/010` hết gap |
...
| 2 | ✅ ~~`C-GIFT-04` vòng 2~~ — **ĐÓNG 2026-09-17**, BA trả lời đủ (1)(2)(3)(4) | — | — |
...
| 4 | 🟡 **Ảnh UI nút "Bạn đã đánh giá" (disable, sau khi tặng)** | BA xác nhận bằng lời nhưng chưa có ảnh đính kèm; vibe-check demo 2026-09-17 không tới được màn này (ô tải ảnh bằng chứng ở bước "Xác nhận đã giao" không mở được file picker trong bản demo) | Xin BA/QA chụp khi có STG thật; không chặn `generate-tc` vì đã có câu trả lời bằng lời |
