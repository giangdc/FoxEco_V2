...
| RISK-ORD-09 | ORD / Ảnh bắt buộc | **(risk mới)** `BR01-01` biến ảnh từ *không rõ* thành **bắt buộc ≥ 1, chặn sang bước 2** — nếu app STG chưa siết thì `SC-ORD-054` (**P1**) FAIL vì *"app chưa cập nhật"*, không phải lỗi logic; và cả 4 SC cụm ảnh sẽ nhiễu theo | **High** | `DOC-v1.1-01` §8.1.1 BR01-01 (trang 33) vs `KP-01` §10 (quan sát app 2026-07) không ghi nhận ràng buộc bắt buộc nào | `SC-ORD-054` (P1) · `SC-ORD-055` · `SC-ORD-056` · `SC-ORD-012` | **Vibe-test 1 lượt wizard bước 1 trước `generate-tc`** — xác nhận app có siết ảnh bắt buộc + trần 5 + bộ đếm `n/5` chưa | Open | REQ-ORD-006, SC-ORD-054, SC-ORD-055, SC-ORD-056 |
