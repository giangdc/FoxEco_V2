...
> **Cập nhật lần cuối: **VR-015** (2026-09-21) — chạy `TC-ASN-006` bằng 1 real + 1 emulator · cùng ngày đổi `008`/`024`/`026` → 🚫 BLOCKED theo QC** · Nguồn scope: `03_test-cases/v1.1/fragments/TC-ASN-v1.1.md` + `03_test-cases/v1.0/fragments/TC-ASN-v1.0.md`
> **Tổng: có verdict cuối 26/26 (23 PASS · 0 FAIL · 3 BLOCKED) · CÒN NỢ 0** — 🔎 **chỉ v1.1 (13 TC): 10 PASS · 0 FAIL · 3 BLOCKED** (`006` `011` `012` `015` `016` `017` `018` `022` `023` `025` PASS · `008` `024` `026` BLOCKED)
>
> 🟢 **2026-09-21 — QC recheck: `TC-ASN-016` và `TC-ASN-025` KHÔNG phải bug ⇒ đổi FAIL → ✅ PASS (sửa Expected, không đổi số lượng TC — §10.5).**
> Quy tắc thực tế: **trần 5 thông báo khớp tuyến tính theo TÀI KHOẢN, mọi tuyến cộng dồn** (QC chốt) — trùng đúng số đo VR-010. Expected của 016/025 đã sửa ở fragment + `TC-MASTER-v1.1.xlsx` (+ `LATEST`).
> ✅ **BA đã xác nhận lại 2026-09-21 (QC GiangDC2 báo lại, chưa có văn bản)** — thay kết luận cũ `C-ASN-04(e)` (2026-09-17: *"trần độc lập theo từng tuyến"*). `TC-ASN-014` / `TC-ASN-017`: caveat *"chạy lại sau fix"* **hết hiệu lực** (không còn bug để fix). ⛔ Đã **bỏ** đề nghị `/log-bug` cho 016/025 — không log.
...
| **VR-015** | 2026-09-21 | **1** | **1P** — `006` *(2 thiết bị; lần chạy đầu không hợp lệ, lần 2 hợp lệ)* |
...
| TC-ASN-006 | ✅ PASS | SC-ASN-006 | P1 | Check hai người vận chuyển nhận gần đồng thời chỉ một người ghép được | VR-015 | `VR-015-ASN-2026-09-21/screenshots/TC-ASN-006__verify-nguoi-thang-vao-theo-doi-don.png` + `VR-015-ASN-2026-09-21/screenshots/TC-ASN-006__verify-nguoi-thua-toast-tin-da-co-nguoi-nhan.png` + `VR-015-ASN-2026-09-21/screenshots/TC-ASN-006__pre-hai-may-cung-o-modal-xac-nhan.png` + `VR-015-ASN-2026-09-21/screenshots/TC-ASN-006__verify-chuoi-4-khung-hinh-ca-hai-may.png` + `VR-015-ASN-2026-09-21/screenshots/TC-ASN-006__verify-chu-tin-thay-1-nguoi-van-chuyen-la-nguoi-thang.png` | 2 thiết bị (real `taipm` + emulator `anhptm17`) bấm `Xác nhận` cách nhau **≤~95 ms** trên cùng 1 tin có sẵn của `stag_giangdc2@`: emulator thắng → `Theo dõi đơn` vai carrier; real thua → toast **"Tin đã có người nhận vận chuyển"**. ⚠️ **Lần chạy đầu (16:13) KHÔNG hợp lệ** — bên thua bị chặn vì *chưa chấp nhận điều khoản*, không phải thua cuộc đua. ✅ **Đã khép vế chủ tin (2026-09-21 16:58):** `stag_giangdc2@` thấy đơn race có `Ghép thành công · 16:22 · Phan Thị Mỹ Anh` (đúng 1 người vận chuyển = người thắng). ⚖️ 1 mẫu ⇒ không thay `TC-ASN-024`. |
...
| TC-ASN-008 | 🚫 BLOCKED | SC-ASN-008 | P2 | Check ba phiên ba vai cùng cập nhật "Đã ghép" trong vòng 5 giây | — | — | **Lý do (QC chốt 2026-09-21): không test được** — cần **3 thiết bị** đồng thời để đo mốc ≤5 giây (`NFR-08`); môi trường chỉ có 2 (1 real + 1 emulator, RAM host không đủ bật emulator thứ 2). Trước đó QC để lại lúc 2026-09-19 16:30. ⛔ Không đổi `DESCOPED` (§10.5) — giữ dòng, chờ có đủ 3 thiết bị. |
...
| TC-ASN-016 | ✅ PASS | SC-ASN-014 | P2 | Check tin OFFER có 5 tin NEED khớp nhận đủ 5 thông báo | VR-010 *(FAIL) → đổi PASS 2026-09-21* | `VR-010-ASN-2026-09-19/screenshots/TC-ASN-016__step3-FAIL-chi-4-thong-bao-moi.png` + `VR-010-ASN-2026-09-19/screenshots/TC-ASN-016__pre-du-5-tin-need-tren-bang-tin.png` | 🟢 **QC recheck 2026-09-21: KHÔNG phải bug** — Expected sửa theo quy tắc *trần 5 tính theo TÀI KHOẢN, mọi tuyến cộng dồn*. Số đo VR-010 khớp Expected mới: 5 tin NEED khớp → **4** thông báo mới + **1** tồn = tổng **5** = đúng trần TÀI KHOẢN. ✅ BA đã xác nhận lại 2026-09-21 (qua QC, chưa có văn bản) — thay `C-ASN-04(e)` cũ. ⚠️ Tên file ảnh còn chữ `FAIL` (đặt lúc chạy VR-010, giữ nguyên để truy vết). *(ghi chú cũ bên dưới đã lỗi thời)* · **Expected 5 · Actual 4.** Bảng tin có **đủ 5** tin NEED khớp (6′·11′·23′·28′·32′, kiểm bằng page source 2 vị trí cuộn) nhưng chuông chỉ thêm **4**. Tổng chuông = 5 = 4 mới + 1 baseline ⇒ nghi **trần 5 tính theo TÀI KHOẢN, không theo TUYẾN** (`C-ASN-04(e)`) — tách giả thuyết ở `TC-ASN-025`. 🪤 `T-ASN-10`: `instance(N)` chỉ thấy node đang render, ⛔ không dùng làm phép đếm tuyệt đối. |
| TC-ASN-017 | ✅ PASS | SC-ASN-014 | P2 | Check tin NEED khớp thứ 6 không sinh thêm thông báo khi tuyến đã đủ trần 5 | VR-010 | `VR-010-ASN-2026-09-19/screenshots/TC-ASN-017__verify-so-thong-bao-khong-doi.png` | Tin NEED thứ 6 (`SEED R1-6`) ⇒ số thông báo giữ nguyên 4, ⛔ không có mục mới (đo 2 lần 17:38 & 17:43). *(cũ)* ⚠️ **KHÔNG chứng minh `C-ASN-04(d)`**: tuyến `R1` mới có 4 thông báo, thứ chặn tin thứ 6 là **trần theo TÀI KHOẢN** ⇒ tiền đề "tuyến đã đủ 5" chưa bao giờ đạt. *(2026-09-21: hết hiệu lực — không còn bug để fix, trần theo tài khoản là quy tắc đúng)* |
...
| TC-ASN-023 | ✅ PASS | SC-ASN-011 | P2 | Check có thông báo khớp tuyến trong vòng 60 giây khi khớp đầy đủ điều kiện | VR-007 | `VR-007-ASN-2026-09-19/screenshots/TC-ASN-023__verify-thong-bao-khop-tuyen.png` | Thông báo *"Tìm thấy đơn hàng phù hợp tuyến của bạn"* trỏ **đúng** tin vừa đăng. 🕐 Độ trễ đo được ∈ **(5s, 65s]** — cận trên vượt 60s **do làm tròn nhãn "3 phút trước"**, ⛔ không phải app chậm. 🔁 Đề nghị đo lại `NFR-04` bằng **2 thiết bị** · ✅ **QC chấp nhận độ trễ đo được (2026-09-21)** — không cần đo lại NFR-04. |
| TC-ASN-024 | 🚫 BLOCKED | SC-ASN-006 | P1 | Check 50 request ghép đồng thời trên cùng 1 tin giữ tỷ lệ trùng 0% | — | — | **QC chốt 2026-09-21: ghi nhận BLOCKED — không test được** *(trước đó khai N-A)*. **Lý do:** cần công cụ **concurrency/load test gọi thẳng API** (JMeter/k6) bắn 50 request đồng thời — fragment `TC-ASN-v1.1.md` ghi rõ *"⛔ không thực hiện bằng thao tác tay trên UI"*, giao automation/backend. ⛔ Ngoài phạm vi `vibe-test` UI thuần |
| TC-ASN-025 | ✅ PASS | SC-ASN-014 | P2 | Check mỗi tuyến OFFER có trần 5 thông báo độc lập, không cộng dồn | VR-010 *(FAIL) → đổi PASS 2026-09-21* | `VR-010-ASN-2026-09-19/screenshots/TC-ASN-025__step3-FAIL-tuyen-2-khong-co-thong-bao.png` + `VR-010-ASN-2026-09-19/screenshots/TC-ASN-025__pre-seed-r2-khop-tuyen-2.png` | 🟢 **QC recheck 2026-09-21: KHÔNG phải bug** — Expected sửa theo quy tắc *trần 5 tính theo TÀI KHOẢN, mọi tuyến cộng dồn*. Số đo VR-010 khớp Expected mới: tuyến OFFER 2 (0/5 slot) + 1 tin NEED khớp → **0** thông báo mới, tổng vẫn **5** = đúng trần TÀI KHOẢN. ✅ BA đã xác nhận lại 2026-09-21 (qua QC, chưa có văn bản) — thay `C-ASN-04(e)` cũ. ⚠️ Tên file ảnh còn chữ `FAIL` (đặt lúc chạy VR-010, giữ nguyên để truy vết). *(ghi chú cũ bên dưới đã lỗi thời)* · **`OFFER-R2` (tuyến hoàn toàn khác, 0/5 slot) + 1 tin NEED trùng khít ⇒ KHÔNG sinh thông báo nào; tổng vẫn đúng 5.** Đo 2 lần (17:38 · 17:43), tin đã 2–7 phút ≫ `NFR-04` 60s ⇒ loại trừ "đến chậm". ⇒ **trần 5 áp theo TÀI KHOẢN, không theo tuyến** — đúng rủi ro `C-ASN-04(e)`. Đây cũng là nguyên nhân gốc của `TC-ASN-016` FAIL. **Cần `/log-bug`.** |
| TC-ASN-026 | 🚫 BLOCKED | SC-ASN-019 | P2 | Check request đọc tin OFFER qua API bởi người ngoài bị chặn và ghi audit log | — | — | **QC chốt 2026-09-21: ghi nhận BLOCKED — không test được** *(trước đó khai N-A)*. **Lý do:** cần **API client** (Postman/tương đương) + token của tài khoản không sở hữu tin, và kiểm **audit log** phía server — fragment ghi rõ *"UI không có đường dẫn tới tin OFFER của người khác"*, giao automation/backend/security test. ⛔ Ngoài phạm vi `vibe-test` UI thuần |

## Tổng hợp

| Verdict | Số | TC |
|---|---|---|
| ✅ PASS | 23 | **`TC-ASN-016`** · **`TC-ASN-025`** · **`TC-ASN-006`** · `TC-ASN-001` · `002` · `003` · `004` · `005` · `007` · `009` · `010` · `011` · `012` · `013` · **`014`** · **`015`** · **`017`** · **`018`** · **`019`** · `020` · `021` · `022` · `023` |
| ❌ FAIL | 0 | — *(`016` · `025` đã đổi PASS 2026-09-21 theo QC recheck)* |
| 🚫 BLOCKED | 3 | `TC-ASN-008` (cần 3 thiết bị) · `TC-ASN-024` (cần công cụ concurrency/API) · `TC-ASN-026` (cần API client + audit log) — **QC chốt 2026-09-21** |
| ⚠️ NOT_EVIDENCED | 0 | — |
| ⏳ NOT_RUN | 0 | — |
| ⛔ N-A | 0 | — |
| **Tổng** | **26** | |

**Có verdict cuối: 26/26 (23P · 0F · 3 BLOCKED) · CÒN NỢ: 0.** Chỉ v1.1: **10P · 0F · 3 BLOCKED / 13**. ⚠️ 3 BLOCKED (`008` `024` `026`) chưa đóng.

---

## ⛔ ĐÃ BỊ BÁC BỎ 2026-09-21 — ~~Kết luận nghiệp vụ của VR-010 — 1 lỗi gốc, 4 TC liên đới~~

> 🟢 **QC recheck 2026-09-21: đây KHÔNG phải bug.** Số đo bên dưới vẫn đúng, nhưng chúng mô tả **quy tắc thực tế** *(trần 5 theo tài khoản)* chứ không phải lỗi. Giữ nguyên để truy vết. ⛔ Không `/log-bug`.
